...
K_ = x(1); B_ = x(2:5); Amp = x(6); tau = x(7); Amp_h = x(8); tau_h = x(9); K2_ = x(10);  gamma = x(11); A_=x(12); C_=x(13); b_dc=x(20); b_dcp=x(21);
...
figure()
CM = ['k','r','g','b'];
dis_cutoff = 11;  % displacement cutoff
...
%     title('dr'); set(gca,'Fontsize',20); set(gcf,'color','w'); ylabel('probability')
    ...
    for bin = 1:numel(bb)-1
        ...
        prob_dr_pr(bin) = sum(gampdf(zeros(1,length(pointsInBin)-1)+bb(bin), ks_(1)+1*phis_(1)*dv_k(pointsInBin(1:end-1))+0, thetas_(1))  .*p_turn_k(pointsInBin(2:end)));
        prob_dr_wv(bin) = sum(gampdf(zeros(1,length(pointsInBin)-1)+bb(bin), ks_(2)+1*phis_(2)*dv_k(pointsInBin(1:end-1)), thetas_(2))  .*(1-p_turn_k(pointsInBin(2:end))));
    end
...
imagesc(M); hold on
CM = ['k','r','g','b'];
exp_wind = (2200:3150)+3000;%[1800:2800]+0; %25000:31000;
...
%% example
wind_ex = offset+(2200:3150)+7000;%2800;  %1200:2200   %800:1200
figure;
subplot(311)
yyaxis left 
