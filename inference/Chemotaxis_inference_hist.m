...
kappa = ((K_)^2)^1;
kappa2 = ((K2_)^2)^1;
if kappa2<0.05; kappa2=0; end  %simplify calculation
A = A_*1;
Kdcp = Amp*exp(-xx/tau);
...
%     dCpv = [Fcon(tracks(t-1,1)+perp_dir(1),tracks(t-1,2)+perp_dir(2)) - Fcon(tracks(t-1,1)-perp_dir(1),tracks(t-1,2)-perp_dir(2)) ,  dCpv];
    ...
    P_event = A / (1+exp( -(sum(Kddc.*dCv)*1. + 1*(sum(Kdth.*abs(dthv)*180/pi)) *dt + 0*base_dc)+0) ) + Pturn_base;%length(wind)
    if rand < P_event*1
        beta = 1;
    else
        beta = 0;
