...
% this is the original file to load from raw tracking data
...
for c = 1:length(Tracks) %length(Paths)
    ...
    trials(1:50) = NaN; trials(end-1:end) = NaN;
...
UB = [200, 1., ones(1,nB)*inf, 0.01, inf, 50, inf, 100, 20, 1];%    inf 5];
% prs0 = rand(1,10);
prs0 = [100, 0.1, randn(1,nB)*10, 0.01, -10, 25, 10, 25, 5, .2];%    0 0]; 
...
% prs0 = [alpha_h, alpha_dc, alpha_dcp, kappa_turn, kappa_wv, gamma, A, C];%
...
[aa,bb] = hist(allas*pi/180,300);
figure;
H = bar(bb,aa/sum(aa),300);
x_data = H.XData;  y_data = H.YData

[aa,bb] = hist(-(ang_fit - filt_dcp - base_dcp)*pi/180 , 300);
...
A = A_*1;
Kdcp = Amp*exp(-xx/tau)*1;
Kdth = Amp_h*exp(-xx_h/tau_h)*1;
Kddc = B_ * cosBasis'*1;
