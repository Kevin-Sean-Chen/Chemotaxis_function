...
for tt=pad:lt
    ...
    F(tt) = dC(tt-pad+1:tt)*K_dc' + abs(dth(tt-pad+1:tt))*r2d*K_h';  % linear filtering
...
%  logpy [T K] - loglikelihood for each observation for each state
    ...
    for k = 1:K
        ...
        logli(:,k) = ( mask.* ( log(marginalP + 1*1e-10) ) ) + lambda*(1*sum((K_dc - 0).^2));% + 0.1*sum((E_ - 0).^2) + 0*C_^2);  % adding slope l2 regularization
    end
end
