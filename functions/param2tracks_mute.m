...
%%% model parameters
%%% this function is different from param2tracks as it takse the nnL5 model
%%% for mutant scans
function [simdata, CI] = param2tracks_mute(x, specs, v_dist, data_inits)
%%%
...
%         tracks(1,:) = [size(M,2)*1/2 + randn()*0, randn()*0 + size(M,1)*1/2]*1. + 0.*[size(M,2)*rand(), size(M,1)*5/8];
        temp = randperm(length(data_inits));
        init_xy = data_inits(temp(1)).xy(:,1);
        tracks(1,:) = init_xy';
        
        tracks(2,:) = data_inits(temp(1)).xy(:,2);
        %tracks(1,:)+randn(1,2)*vm*dt;%origin+randn(1,2)*vm*dt;
        ...
        for t = 2:T
            ...
            P_event = (A - Pturn_base) / (1+exp( -(sum(Kddc.*dCv)*1. + (sum(Kdth.*abs(dthv)*1)) *dt + base_dc*1)+0) ) + Pturn_base;%length(wind)
...
%     ci1 = 0;  ci2 = 0;
%     for rep = 1:REP
%         dC = simdata(rep).dc(end) - simdata(rep).dc(1);
%         c0 = simdata(rep).dc(1);
%         if  dC > 0
%             cii = cii + dC/(100-c0);
%             ci1 = ci1 + 1;
