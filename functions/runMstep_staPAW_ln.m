function mm = runMstep_staPAW_ln(mm, xx, yy, gams, mask)
% mm = runMstep_LinGauss(mm,xx,yy,gams)
%
% Run m-step updates for staPAW observation model (test with log-normal speed!)
%
% Inputs
% ------
...
% opts.Algorithm = 'sqp';
LB = [1e-0, ones(1,nB)*-inf, -inf, 1e-0, -inf, 1e-1, 1e-0,   0.  0   0    -100 -100     0     0 -50 -50 -10 -10];
...
function [nll] = nll_staPAW(THETA, dth, dcp, dc, dv, gams, Basis, lambda, mask)
    ...
    ln_mu = THETA(14:15);    % shape parameters for gamma
    ln_sig = THETA(16:17);    % scale parameters for gamma
%     ln_AR = THETA(18:19);        % AR weight for gamma
...
%     VM = C * exp(kappa_wv^2*cos(( dth(2:end) - filt_dcp - base_dcp)*d2r));
    ln_wv = lognpdf(dv(2:end), ln_mu(2) + ln_AR(2)*dv(1:end-1), ln_sig(2));
    ...
    ln_pr = lognpdf(dv(2:end), ln_mu(1) + ln_AR(1)*dv(1:end-1), ln_sig(1));
    
    %%% marginal probability
    marginalP = (1-P).*VM.*ln_wv + P.*VM_turn.*ln_pr;
