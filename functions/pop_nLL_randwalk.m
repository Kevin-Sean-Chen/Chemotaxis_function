function [NLL] = pop_nLL_randwalk(THETA, D)
%%%
...
%%%
    ...
    for nn = 1:ntracks
        ...
        NLL = NLL + nLL_randomwalk(THETA, D(nn).dth, D(nn).dcp, D(nn).dc, D(nn).Basis, D(nn).lambda, D(nn).mask);%/Z; % sum across tracks normalize by time
    end
