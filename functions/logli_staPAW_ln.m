function [logli] = logli_staPAW_ln(mm, xx, yy, mask)

% Compute log-likelihood term under a mixture of von Mesis angle +
% log-normal speed model; both under a shared sigmoid
%
% Inputs
% ------
...
%  logpy [T K] - loglikelihood for each observation for each state
    ...
    for k = 1:K
        ...
        ln_mus = THETA(1,14:15,k);       % shape parameters for speed distribution
        ln_sigs = THETA(1,16:17,k);       % scale parameters for speed distribution
%         ln_AR = THETA(1,18:19,k);           % AR weight for speed distribution
...
%         VM = C * exp(kappa_wv^2*cos(( dth(2:end) - filt_dcp - base_dcp)*d2r));
        logn_wv = lognpdf(dv(2:end), ln_mus(2) + ln_AR(2)*dv(1:end-1), ln_sigs(2));
        ...
        logn_pr = lognpdf(dv(2:end), ln_mus(1) + ln_AR(1)*dv(1:end-1), ln_sigs(1));

        %%% marginal probability
        marginalP = (1-P).*VM.*logn_wv + P.*VM_turn.*logn_pr;
%         pos = (~isinf(log(dv)));
